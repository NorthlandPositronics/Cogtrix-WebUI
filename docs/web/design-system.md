...
| SIM-001 | **Open (v3.16, 2026-03-27)** — `docs/web/mockups/assistant-testing-tab.svg` required before `web_coder` begins implementing `SimulatorPanel`. The mockup must include: (a) full form at `lg+` viewport with the advanced-options collapsible open, (b) `persist` checkbox checked with always-visible amber warning banner visible, (c) result card showing a guardrail-blocked outcome (Guardrail blocked badge active, amber guardrail reason callout visible), (d) result card showing a clean inbound response (all badges inactive except a green "Memory persisted" if relevant). See `docs/web/briefs/testing-tab-brief.md` for the complete graphic designer brief. | High | Yes — blocks `web_coder` | `graphic_designer` produces SVG → `web_designer` reviews → `web_coder` implements. |
